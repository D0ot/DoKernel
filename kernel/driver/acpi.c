...
{
    uint8_t *ebda_addr = (uint8_t*)(uint32_t)*(uint16_t*)(PTR_2_EBDA);

    for(uint32_t i = 0; i != 0x400; i += 0x10)
    {
        if(strmatch((char*)(ebda_addr + i), "RSD PTR ", 8))
        {
            return (ebda_addr + i);
        }
    }

    for(uint8_t *p = (uint8_t*)ANOTHER_RSDP_START; p != (uint8_t*)(ANOTHER_RSDP_END); p += 0x10)
    {
        if(strmatch((char*)p, "RSD PTR ", 8))
        {
            return p;
        }
    }
...
RSDP_Struct getRSDP_Struct(uint8_t *address)
{
    ...
    if(rsdp.revision >= 2)
    {
        ...
        if(checksum((uint8_t*)&rsdp, sizeof(rsdp)) != rsdp.extendedchecksum)
        {
